...
public class UpdateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        MemberVO updatedMember = new MemberVO(userid, password, email, emailAgree, interest, phone, introduce);

        // MemberDAO를 통해 회원 정보를 업데이트합니다.
        int result = dao.update(userid, updatedMember);
        System.out.println(updatedMember);
        if (result == 1) {
        
        	response.sendRedirect("/Homepage_MJH/select.do");
